from datetime import datetime
from bson import ObjectId

class AutoAnalyticalModel:
    RULE_TYPE_PRODUCT_CATEGORY = 'product_category'
    RULE_TYPE_PRODUCT = 'product'
    RULE_TYPE_CONTACT = 'contact'
    RULE_TYPE_AMOUNT_RANGE = 'amount_range'
    
    RULE_TYPES = [RULE_TYPE_PRODUCT_CATEGORY, RULE_TYPE_PRODUCT, RULE_TYPE_CONTACT, RULE_TYPE_AMOUNT_RANGE]
    
    def __init__(self, data=None):
        if data:
            self._id = data.get('_id')
            self.name = data.get('name')
            self.description = data.get('description')
            self.rule_type = data.get('rule_type')
            self.rule_value = data.get('rule_value')
            self.analytical_account_id = data.get('analytical_account_id')
            self.priority = data.get('priority', 0)
            self.is_active = data.get('is_active', True)
            self.created_by = data.get('created_by')
            self.created_at = data.get('created_at', datetime.utcnow())
            self.updated_at = data.get('updated_at', datetime.utcnow())
        else:
            self._id = None
            self.name = None
            self.description = None
            self.rule_type = None
            self.rule_value = None
            self.analytical_account_id = None
            self.priority = 0
            self.is_active = True
            self.created_by = None
            self.created_at = datetime.utcnow()
            self.updated_at = datetime.utcnow()
    
    def to_dict(self):
        return {
            '_id': str(self._id) if self._id else None,
            'name': self.name,
            'description': self.description,
            'rule_type': self.rule_type,
            'rule_value': self.rule_value,
            'analytical_account_id': str(self.analytical_account_id) if self.analytical_account_id else None,
            'priority': self.priority,
            'is_active': self.is_active,
            'created_by': str(self.created_by) if self.created_by else None,
            'created_at': self.created_at.isoformat() if self.created_at else None,
            'updated_at': self.updated_at.isoformat() if self.updated_at else None
        }
    
    def to_db_dict(self):
        return {
            'name': self.name,
            'description': self.description,
            'rule_type': self.rule_type,
            'rule_value': self.rule_value,
            'analytical_account_id': ObjectId(self.analytical_account_id) if self.analytical_account_id else None,
            'priority': self.priority,
            'is_active': self.is_active,
            'created_by': ObjectId(self.created_by) if self.created_by else None,
            'created_at': self.created_at,
            'updated_at': self.updated_at
        }
    
    @staticmethod
    def from_db(data):
        if data:
            return AutoAnalyticalModel(data)
        return None
